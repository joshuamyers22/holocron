...
| Malicious serialized model | Code execution or silent model drift | No supported pickle interchange; strict versioned data-only JSON, duplicate/non-finite rejection, 64 MiB document limit, shape limits, and cross-field validation | ADR-008; serialization acceptance record; serialization tests | joshuamyers22 |
...
| Pathological resampling request | Denial of service or partial evidence presented as complete | Observation/split/total-index bounds, explicit failures, retained completion counts, and complete/partial/failed status | `src/holocron/validation/resampling.py`; `tests/test_resampling.py`; Phase 6 resampling acceptance record | joshuamyers22 |
